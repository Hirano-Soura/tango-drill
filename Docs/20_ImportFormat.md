...
| `tango-drill/v1` | JSON の `format` 欄 | ファイルでの受け渡し・教材セット。バックアップの語の部分も同じ形で書く | `tango-drill_v1.json` |
...
- バックアップ(`format` が `tango-drill-backup/` で始まる JSON)は語の取り込みでは読まず、バックアップの読み込みへ案内する。
  バックアップは記録ごと置き換える別の経路で、版も別に持つ([22_Storage.md](22_Storage.md))
...
- 画面が必ず確認表を通すことは `tests/e2e/import.spec.js`(Playwright)で確かめる。画面での操作の規則は [23_Screens.md](23_Screens.md) §3
- 追加タブで手で入れた 1 語も同じ確認表を通す(`addOneWord`)。入力欄そのものが確認に当たるので、
  行が「足す」のときだけ確定し、それ以外は足さずに編集へ案内する([23_Screens.md](23_Screens.md) §3)
- 単語帳の全体(追加日・記録・印を含む)に当てるときは `importIntoBook`(`core/book.js`)を使う。
  中で `applyImport` を呼ぶので、確認表を経ることは変わらない
- バックアップの読み込みは確認表を通さない。自分で書き出したファイルで全体を置き換える操作であり、
  AI の誤出力を単語帳に混ぜる経路ではないため(INV-2 の対象外)。代わりに件数を見せて確認し、元に戻せる([22_Storage.md](22_Storage.md) §3)
...
- 品詞を埋めると語の鍵が変わる。`applyImport` は変わった鍵の組(`rekeys`)を返し、
  `importIntoBook` がそれで追加日・記録・印を新しい鍵へ移す(`moveKeys`。[22_Storage.md](22_Storage.md) §4)
...
- **例文(`ex`)・和訳(`exJa`)・出どころ(`exSrc`)は一組で動く**
  ...
  - 単語帳の側でも、例文の無い和訳・出どころは持たない前提とする(持っていると、例文を埋めるときに黙って消える)。
    画面の編集(`editWord`)もこれを守る([23_Screens.md](23_Screens.md) §3)
