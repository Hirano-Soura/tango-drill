...
| P-8 | Playwright の `setInputFiles` にファイルのパスを渡しても、ファイルを選んだことにならない(`change` が起きず、エラーも出ない) | パスに日本語を含むと起きる。ASCII だけのパスでは起きない(実測 2026-09-25 / Playwright 1.63.0・Chromium 141(`PW_EXECUTABLE`)・Linux / `日本語` と `a。b` のディレクトリの 2 通りとも再現し、同じ中身を置いた `ascii` のディレクトリでは通った。Edge・Windows では未確認)。`testInfo.outputPath` は試験名をディレクトリ名に含むので、日本語の試験名だとここに当たる | `{ name, mimeType, buffer }` で中身と名前を渡す(`tests/e2e/backup.spec.js`)。選んだあとの画面の変化を必ず確かめる(何も起きないことを「断った」と読まない) |
| P-9 | 画面の確認が、たまにだけ落ちる(同じ入力を 2 回続けて追加する確認が、全体で回すと 3 回に 1 回ほど落ちた。単独で 30 回回すと落ちなかった) | 単語帳を変える操作は、保存(非同期)が終わってから描き直す(`commit`)。完了を待たずに次の入力を始めると、あとから来た描き直しが入力の途中で欄を空にする | 単語帳を変えるボタンを押したら、次の操作の前にその結果(メッセージなど)が出るのを `expect` で待つ(`tests/e2e/flow.spec.js` の重複の追加) |
